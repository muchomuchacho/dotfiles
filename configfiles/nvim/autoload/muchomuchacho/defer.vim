...
function! muchomuchacho#defer#defer(evalable) abort
  if has('autocmd') && has('vim_starting')
    ...
    execute 'autocmd User MuchoDefer ' . a:evalable
  else
    execute a:evalable
  endif
endfunction
...
function! muchomuchacho#defer#packadd(pack, plugin) abort
  execute "call muchomuchacho#defer#defer('call muchomuchacho#plugin#packadd(\"' . a:pack . '\", \"' . a:plugin . '\")')"
endfunction
